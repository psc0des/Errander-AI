...
> **MCP is an _allowed_ Layer A capability, not a built one.** There is no MCP client or server anywhere in the codebase and no MCP dependency in `pyproject.toml`. Today Layer A (the deterministic `--ask` operator assistant and the daily probe) reaches Prometheus and ELK over direct HTTP (`aiohttp`) and the audit DB over SQL (`asyncpg`/SQLAlchemy), as plain in-process Python calls. The "may use ... MCP" wording above is a **boundary rule** — *if* MCP is ever adopted it belongs in Layer A only, never the Layer B execution path — and a forward-looking allowance for plugging in **external, third-party** operator tools (e.g. a Grafana or GitHub MCP). It does not describe current wiring. For Errander's own data, direct clients are the deliberate choice (in-process, no trust boundary to cross, no extra egress surface).
