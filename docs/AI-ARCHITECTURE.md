...
> **As-built note — this table is the _permission boundary_, not a build manifest.** It says what Layer A *may* use; it does not say these integrations exist. As shipped, Layer A (the deterministic `--ask` Operator Assistant and the daily probe) reaches **Prometheus and ELK over direct HTTP (`aiohttp`)** and the **audit DB over SQL (`asyncpg`/SQLAlchemy)** — plain in-process Python calls. **There is no MCP client or server in the codebase, and no MCP dependency.** "MCP servers", "CLIs over SSH", and "CVE/runbook Skills" in the rows above are *permitted future* integration paths — MCP in particular is the right tool only for **external, third-party** operator systems Errander doesn't own (a Grafana MCP, a GitHub MCP). For Errander's own data, direct clients are the deliberate choice: in-process, no trust boundary to cross, no added egress surface (which matters for the no-egress self-hosted-vLLM deployment target). (An agentic `--ask --agentic` loop and a `/ui/chat` console were prototyped on this allowance and then removed — deferred to a separate conversational-investigation project; see README's Roadmap.) The boundary rule still holds regardless of mechanism: whatever Layer A uses, it stays read-only and never crosses into the Layer B execution path.
...
Phase 1 (shipped) is deterministic-only: the detector proposes `disk_cleanup` / `log_rotation` from disk-growth signals and surfaces drift / login spikes as review-only proposals. Phase 2 (shipped, opt-in `--ask --agentic`) adds a bounded, read-only Layer A tool-calling loop that answers operator questions and may recommend LOW-risk work. Phase 3 (shipped, default OFF) lets the daily probe launch that same loop **on its own** for VMs it just flagged — no operator has to ask — *enriching* the Phase 1 proposal with correlated evidence rather than replacing it; still Layer A, still never executing, and a failed/unavailable LLM simply leaves the deterministic proposal exactly as filed (zero extra cost). Phase 4 (shipped) gives the whole system memory: reject the same `(vm_id, action_type)` twice and further auto-proposals for that pair pause for 14 days — enforced at a single shared write path so the detector, the trigger, and the agentic filer can't drift apart or bypass each other — and the same rejection/approval/execution history now informs both investigation paths so recommendations are made with awareness of what's already been tried. Full design and phasing: [`../tasks/fable-plan.md`](../tasks/fable-plan.md); pipeline diagram: [`diagrams/detect-and-propose.md`](diagrams/detect-and-propose.md).
