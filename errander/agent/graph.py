...
async def plan_vm_node(
    ...
    *,
    ...
    Runs OS detection via SSH (read-only), then computes a deterministic
    action plan and an advisory LLM planning note for inclusion in the
    ImmutableBatchPlan. The plan itself never depends on the LLM (R1) — only
    the informational `ai_note` does.
...
def _format_plan_for_approval(
    ...
    # AI analysis — informational only, placed last so Slack's truncation
