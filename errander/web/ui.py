"""Production web UI for Errander-AI.

Dashboard, batch/VM history, approvals, settings, inventory management,
docker_hygiene object-level web approval, AI-decision audit, and controller
monitoring pages.

Builds an aiohttp application (:func:`build_ui_app` / :func:`start_web_server`)
intended to run as its own OS process (``python -m errander.web``), separate
from the agent process started by ``python -m errander``. Prometheus
metric objects themselves are defined in :mod:`errander.observability.metrics`
and re-exported here for the monitoring page.

This module MUST NOT import from ``errander.execution`` or ``errander.agent.*``
— enforced by ``tests/web/test_import_isolation.py``. The web process has no
SSH keys and no code path to the executor.
"""
...
AUDIT_STORE_KEY: web.AppKey[AuditStore | None] = web.AppKey("audit_store")
APPROVAL_STORE_KEY: web.AppKey[ApprovalRequestStore | None] = web.AppKey("approval_store")
HYGIENE_STORE_KEY: web.AppKey[HygieneApprovalStore | None] = web.AppKey("hygiene_store")
OVERRIDES_STORE_KEY: web.AppKey[OverridesStore | None] = web.AppKey("overrides_store")
BASE_INVENTORY_KEY: web.AppKey[list[VMTarget]] = web.AppKey("base_inventory")
...
#: HMAC secret for verifying signed docker_hygiene approval URLs (None = signed-URL routes disabled).
SIGNING_SECRET_KEY: web.AppKey[str | None] = web.AppKey("signing_secret")

#: True when running behind nginx Mode 2 (public internet) — mandates TOTP for admin group.
PUBLIC_MODE_KEY: web.AppKey[bool] = web.AppKey("public_mode")
...
#: Short-lived cookie bridging password verification → TOTP step (public mode).
#: Holds a signed (username, pending-setup-secret) pair — never a session.
_MFA_PENDING_COOKIE = "errander_mfa_pending"
...
def _make_mfa_pending_token(secret: str, username: str, setup_secret: str | None) -> str:
    """Sign a short-lived pending-MFA token (password verified, TOTP pending).

    ``setup_secret`` carries a freshly-generated TOTP secret when the user has
    none yet (first-time setup); empty string once a secret already exists.
    """
    import hashlib
    import hmac
    import time

    expires = int(time.time()) + _MFA_PENDING_TTL_SECONDS
    msg = f"{expires}:{username}:{setup_secret or ''}"
    sig = hmac.new(secret.encode(), msg.encode(), hashlib.sha256).hexdigest()
    return f"{msg}:{sig}"


def _verify_mfa_pending_token(secret: str, token: str) -> tuple[str, str | None] | None:
    """Verify a pending-MFA token; returns (username, setup_secret) or None."""
    import hashlib
    import hmac
    import time
    ...
    if len(parts) != 4:
        return None
    expires_s, username, setup_secret, sig = parts
    msg = f"{expires_s}:{username}:{setup_secret}"
    ...
    try:
        ...
        return None
    return username, (setup_secret or None)
...
async def _session_auth_middleware(
    ...
    """
    if not request.path.startswith("/ui") or request.path in ("/ui/login", "/ui/login/totp"):
    ...
    if request.method in ("GET", "HEAD") and request.app.get(LOOPBACK_BIND_KEY, True):
        request["user"] = None
        return await handler(request)  # type: ignore[operator, no-any-return]
    raise web.HTTPForbidden(
        reason=(
...
def _csrf_token(request: web.Request) -> str:
    ...
    secret = request.app.get(CSRF_SECRET_KEY, "")
...
async def _ui_settings_get(request: web.Request) -> web.Response:
    ...
    store: OverridesStore | None = request.app.get(OVERRIDES_STORE_KEY)
...
async def _ui_settings_post(request: web.Request) -> web.Response:
    ...
    store: OverridesStore | None = request.app.get(OVERRIDES_STORE_KEY)
    audit: AuditStore | None = request.app.get(AUDIT_STORE_KEY)
...
async def _ui_inventory_get(request: web.Request) -> web.Response:
    ...
    store: OverridesStore | None = request.app.get(OVERRIDES_STORE_KEY)
    base_inventory: list[VMTarget] = request.app.get(BASE_INVENTORY_KEY) or []
    approval_store: ApprovalRequestStore | None = request.app.get(APPROVAL_STORE_KEY)
    pending_count = await approval_store.count_pending() if approval_store is not None else 0
...
async def _ui_dashboard(request: web.Request) -> web.Response:
    store: AuditStore | None = request.app.get(AUDIT_STORE_KEY)
    approval_store: ApprovalRequestStore | None = request.app.get(APPROVAL_STORE_KEY)
...
async def _ui_approvals(request: web.Request) -> web.Response:
    ...
    ai_store: AIDecisionStore | None = request.app.get(AI_DECISION_STORE_KEY)
    ...
    hygiene_store_val: HygieneApprovalStore | None = request.app.get(HYGIENE_STORE_KEY)
    hygiene_section = ""
...
async def _ui_hygiene_approve_get(request: web.Request) -> web.Response:
    ...
    hygiene_store_get: HygieneApprovalStore | None = request.app.get(HYGIENE_STORE_KEY)
...
async def _ui_hygiene_approve_post(request: web.Request) -> web.Response:
    ...
    hygiene_store_post: HygieneApprovalStore | None = request.app.get(HYGIENE_STORE_KEY)
...
async def _ui_ai_decisions(request: web.Request) -> web.Response:
    store: AIDecisionStore | None = request.app.get(AI_DECISION_STORE_KEY)
    approval_store: ApprovalRequestStore | None = request.app.get(APPROVAL_STORE_KEY)
    pending_count = await approval_store.count_pending() if approval_store is not None else 0
...
def build_ui_app(
    *,
    ...
    """Build the web UI application (middlewares + routes + app state).

    Used by :func:`start_web_server` and directly by tests to drive the full
    ...
    app[AUDIT_STORE_KEY] = audit_store
    app[APPROVAL_STORE_KEY] = approval_store
    app[HYGIENE_STORE_KEY] = hygiene_store
    app[OVERRIDES_STORE_KEY] = overrides_store
    app[BASE_INVENTORY_KEY] = base_inventory or []
    ...
    app[PUBLIC_MODE_KEY] = public_mode
...
async def start_web_server(
    ...
    *,
    ...
    """Start the web UI HTTP server (its own OS process, port 9091 by default).

    Serves all ``/ui/*`` pages plus ``/metrics`` and ``/health`` for this
    process's own liveness/monitoring. See :func:`build_ui_app` for the
    full route table.
    ...
    """
    is_loopback = bind_address in ("127.0.0.1", "::1", "localhost")
    user_count = await user_store.count_users() if user_store is not None else 0
    if not is_loopback and user_count == 0:
        msg = (
    ...
    await site.start()
    logger.info(
        "Web UI listening on %s:%d (/ui, /metrics, /health)",
        bind_address, port,
    )
    return runner
