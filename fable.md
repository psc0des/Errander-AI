...
- [ ] `errander/safety/approval.py` — remove the Slack reaction path from decisions (`poll_approval` reaction logic, reaction race, reject-priority); decision waiting moves to the `approval_requests` store poll (§8b)
- [ ] `errander/agent/graph.py` — `approval_gate_node`: post notify+link message instead of reaction prompt
- [ ] Web UI (`errander/observability/metrics.py` / `errander/web/`) — users + groups tables (PostgreSQL-compatible); login flow change from shared credential; RBAC middleware (reader = GET-only pages, admin = decisions + user management); approval handler records username + group; admin page or CLI for user management; TOTP support gated on public mode
- [ ] `deploy/` or SETUP.md — nginx reference config for Mode 2 (TLS, HSTS, rate-limit, localhost upstream, optional IP allowlist)
- [ ] `errander/integrations/slack.py` — keep posting/alerts; reactions API can remain for non-decision uses or be removed
- [ ] `errander/config/settings.py` — remove/deprecate reaction-poll settings; add user-management settings if needed
- [ ] Migrations — users/groups/sessions tables (PostgreSQL-compatible types per v2 rule)
- [ ] Tests — decision only possible via authenticated UI; unauthenticated/reader-role cannot decide; `decided_by` = username in audit; timeout auto-reject unchanged; signed URL grants no authority without session
- [ ] Doc sync: README, SETUP.md (mobile VPN section), RUN.md, AGENTS.md/CLAUDE.md risk-tier table wording ("human approval — Web UI; Slack notifies"), OBSERVABILITY.md, learning doc
