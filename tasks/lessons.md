...
1. **Unencrypted writes.** Every call that writes a secret to a file must go through the encrypt-if-enabled path. `ERRANDER_ELK_API_KEY` was written raw — `encrypt_val()` existed and was used for 3 other secrets, but ELK was missed. Grep: `grep 'echo "ERRANDER_' configure.sh` and verify each secret is the `_env_*` encrypted form.

2. **Visible input.** Every prompt that collects a secret must use `read -rs` (no-echo). `prompt_val` echoes characters; `prompt_secret` doesn't. Both vLLM and "Other" provider API keys, and both ELK API key paths, were using `prompt_val`. Grep: `grep "prompt_val.*[Kk]ey\|prompt_val.*[Tt]oken\|prompt_val.*[Ss]ecret" configure.sh`.

3. **Missing generation.** Machine-random secrets (signing keys, HMAC secrets) should be auto-generated by the wizard, not left as "set this manually" docs. `ERRANDER_SIGNING_SECRET` was documented in CLAUDE.md and docs/SECRETS.md but never written by configure.sh — docker_hygiene would fail loudly at runtime whenever `WEB_BASE_URL` was set.

**How to apply:** Before calling configure.sh complete, run these checks: (a) verify every `ERRANDER_*` write in the `.env` block is the `_env_*` encrypted variable; (b) grep every API/token/password/secret prompt for `prompt_val` and change to `prompt_secret`; (c) grep every env var in CLAUDE.md's secrets block and confirm configure.sh writes it — generating, prompting, or at minimum a commented placeholder.
...
## 2026-07-07 — detect-and-propose: refine the 2026-06-23 removal, don't reverse it

When re-introducing agentic behaviour after the 2026-06-23 chat/investigation removal, the distinction that keeps the record coherent: the **chat surface** stays out of core (a conversational "system of insight" with its own credential model, iteration speed, and eval needs), but the **investigate→propose loop** comes *into* core — because it terminates in the core's own approval pipeline. It is the front end of a system of action, not a system of insight. Framed that way it's a refinement of the June 23 decision, not a flip-flop.

**Rule**: when a past decision removed two things bundled together for one stated reason, re-examine whether the reason applies equally to both before citing it to block re-adding either. Here "complicated the core for unvalidated demand" was true of the chat UI and false of the proposal bridge (which *simplifies* the value story: evidence-quality approvals).

## 2026-07-07 — latent circular import exposed by a new module's collection order

Adding `errander/safety/proposal_store.py` + its tests surfaced a pre-existing circular import: `errander.safety.validators` (module-level `import ...subgraphs.disk_cleanup`) → `subgraphs/__init__` → `subgraphs.patching` (module-level `import ...safety.validators`). `import errander.safety.validators` fails standalone on clean HEAD; the full suite only passed because some earlier test imported the subgraph chain first and populated `sys.modules`. New test files shifted collection order and exposed it as ~180 cascading errors in the alphabetically-last `tests/web/`.

**Rule**: fix latent import cycles at the source (make the *weakest* edge a function-level import — here `validate_no_pkg_lock` in `patching.py`, used in exactly one node), don't paper over them by reordering tests. A green suite that depends on import order is a landmine; the next new module steps on it. Verify the fix with `python -c "import <the module that failed> "` standalone, not just via pytest.

## 2026-07-07 — the full `uv run pytest` is RED on the Windows dev box for pre-existing reasons

Before spending time "fixing failing tests" after a change, check whether the failure is pre-existing on THIS machine. Proven via `git stash -u` (stash tracked + untracked) then full run on clean HEAD: clean HEAD produces **10 failed / 171 errors** — a `tests/web` "RuntimeError" cascade under pytest-asyncio (Windows event-loop teardown), plus stale post-R2 `tests/ui/test_approval_ui.py` tests (they assert pre-RBAC 302/`decided_by="ui:ui"` but the app now correctly returns 403 in zero-users bootstrap mode) and browserless Playwright tests. The "~2463 passed" green baseline is CI/Linux, not this box.

**Rule**: verify a change by running the *touched* test areas in isolation (they pass here), not the full suite — the full suite's red is environmental noise on Windows. Confirm causation with `git stash -u` + full run on clean HEAD before assuming a new change broke anything. A separate lock-based cascade (a failing test leaves an idle-in-transaction backend → next test's `TRUNCATE CASCADE` deadlocks) is real but distinct from the dominant RuntimeError cascade; hardening `_clean_test_db` to reap leaked backends fixes the lock one but not the RuntimeError one, and "terminate all non-self connections" is an xdist footgun — left unfixed pending a proper CI-side look.

## 2026-07-07 — a real lightweight fallback class caught what a mock hid (Phase 3)

Phase 2's tests for `InvestigationAgent.investigate_agentic` mocked the `fallback` param entirely (`AsyncMock()`), so they never exercised the real call `fallback.investigate(**fallback_kwargs)` against a concrete method signature. Phase 3 introduced `NoOpFallback`, a real lightweight class — and its first real test run immediately failed: `NoOpFallback.investigate() missing 1 required positional argument: 'question'`, because the trigger passes `fallback_kwargs={}` (no `question` key) while `NoOpFallback.investigate(self, question: str, ...)` required it.

**Rule**: when a parameter's type is loosened to a Protocol specifically so a *new, real* lightweight implementation can satisfy it (not just to satisfy a mock), write at least one test that exercises the real implementation end-to-end before assuming the mock-based tests already proved the contract. Mocks accept any call shape; real classes don't — that gap is exactly where these bugs hide. Fixed by giving the unused `question` param a default (`= ""`).

## 2026-07-07 — resource lifecycle ordering: don't close what a later step still needs

The probe's `try/finally` closed the Prometheus/ELK clients immediately after `run_env_probe()` returned, before `_detect_and_file_probe_proposals` or (new in Phase 3) the investigation trigger ran. Phase 3 needed those same clients (for `get_vm_metrics`/`search_vm_errors` tools) — since they were already closed by the time the trigger ran, reusing them would have failed silently or raised on the closed aiohttp session. Fixed by widening the `try` block to cover detection + trigger, moving `finally: close prom/elk` to the true end of the resource's usable lifetime.

**Rule**: when adding a new consumer of an existing scoped resource (a client, a connection, a session), trace the resource's full close boundary first — grep the `finally`/`__aexit__`/`.close()` call sites — before assuming "it's already in scope" is enough. Being in scope syntactically doesn't mean it's still open at that point in the control flow.

## 2026-07-09 — reusing a client-generated proposal_id across a reject-cycle loop causes an IntegrityError

`AgentProposal.proposal_id` is assigned once, at construction (`default_factory=uuid4`). A test loop written as `for _ in range(2): stored, _ = await store.create_or_refresh(candidate); await store.decide(stored.proposal_id, approved=False, ...)` — reusing the SAME `candidate` object each iteration — fails on the second iteration with a Postgres `IntegrityError`. Why: the first `create_or_refresh` inserts a row with `candidate.proposal_id` and status `pending`. `decide()` flips it to `rejected`, which drops it out of the partial unique index (`WHERE status='pending'`) that the dedup `ON CONFLICT` targets. The second `create_or_refresh` call, given the same object, attempts a fresh `INSERT` with the SAME `proposal_id` — but that id is already a primary key from the first insert, and `ON CONFLICT` only covers the partial index, not the PK, so Postgres raises instead of upserting.

**Rule**: when a test simulates "the same detection recurring N times" (reject-twice, refresh-N-times, etc.), construct a FRESH model instance each iteration (or re-call the pure detector function), never reuse one object across a create→decide cycle. This matches real production behavior too — a detector never reuses a stale candidate object across probe runs; each run builds new ones. Caught immediately by writing real integration tests against the actual store (not mocks) — another instance of "a real lightweight implementation catches what a mock would hide," same lesson as 2026-07-07's `NoOpFallback` gotcha.

## 2026-07-09 — when something outside your diff also breaks, stop looking in your diff

Mid-Phase-5, `uv run python -m errander --help` started hanging indefinitely on repeated invocations — before I'd even confirmed whether the new eval code was the cause. Bisection order that actually worked: (1) confirm `--help` alone hangs (rules out any Phase 5 logic — it hadn't run yet), (2) `-X importtime` to find *where* the hang starts (right after `platform`/`_wmi`), (3) run a tool that shares the toolchain but does zero runtime `import errander.*` as a control (`ruff`, `mypy` — both stayed green throughout), (4) confirm the suspect subsystem directly (`Get-CimInstance Win32_OperatingSystem` hung too — WMI itself, not my code). Root cause: `sqlalchemy`'s import-time OS probe (`platform.win32_ver()`) blocking on an unresponsive Windows WMI service, unrelated to anything written this session.

**Rule**: when a change you just made appears to break something, first check whether a completely unrelated, zero-diff invocation (`--help`, an empty test, a control binary in the same toolchain) *also* breaks. If it does, the bug is environmental/upstream, not yours — stop reading your diff and start bisecting at the interpreter/OS level (`-X importtime` for Python import stalls; a non-Python control tool to rule out "is it even Python's fault"). Chasing a phantom logic bug in fresh code costs far more than 5 minutes of control-group thinking up front. Also: don't guess root cause from spatial/temporal proximity alone — I initially assumed leftover `Start-Job` processes from my own debugging were the pileup cause; that theory died the moment `Stop-Process` on them also returned "Access is denied" (same as the real blocker, service-level WMI permissions) — verify a causal theory against a concrete test before reporting it as the answer.

## 2026-07-09 — `timeout N` around a long pytest run silently truncates output, not just the process

Wrapping `uv run pytest tests/` (2637 tests, ~490s) in `timeout 300 ... | tail -N` produced a plausible-looking but wrong result twice: the shell's `timeout` killed pytest at ~60% and `tail` still emitted rows, with no `EXIT:124` visible near the top of a `tail`-truncated view. It looked like the suite had "finished" partway through. Only writing to a full log file (`> file 2>&1`, no `tail` mid-pipe) and checking `echo EXIT:$?` at the top surfaced the real exit code.

**Rule**: when timing a long-running command whose true duration is unknown, don't guess a `timeout` value and pipe through `tail` — either omit the internal `timeout` and rely on the tool's own (much larger) timeout parameter, or pick a timeout you're confident exceeds the run, and always capture the exit code explicitly rather than inferring completion from partial piped output. A `tail`-truncated view of a killed process and a `tail`-truncated view of a completed one look identical unless you check the exit code.
