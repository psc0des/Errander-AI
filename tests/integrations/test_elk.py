...
# ---------------------------------------------------------------------------
# search() — arbitrary-terms read-only log search (investigation-agent tool path)
# ---------------------------------------------------------------------------


def _search_hits_response(hits: list[dict[str, object]], status: int = 200) -> MagicMock:
    resp = MagicMock()
    resp.status = status
    resp.json = AsyncMock(return_value={"hits": {"hits": hits}})
    resp.__aenter__ = AsyncMock(return_value=resp)
    resp.__aexit__ = AsyncMock(return_value=False)
    return resp
...
@pytest.mark.asyncio
async def test_search_returns_messages() -> None:
    hits: list[dict[str, object]] = [
        {"_source": {"@timestamp": "2026-06-22T00:00:00Z", "message": "connection refused"}},
        {"_source": {"@timestamp": "2026-06-22T00:01:00Z", "message": "timeout waiting for db"}},
    ]
    resp = _search_hits_response(hits)
    client = ElkClient("http://elk:9200")

    with patch.object(client, "_get_session", return_value=_mock_elk_session(resp)):
        result = await client.search("web-01", ["connection", "timeout"])
...
@pytest.mark.asyncio
async def test_search_builds_query_against_fixed_index_never_caller_controlled() -> None:
    """query_terms/host/level may only shape filter clause content — the
    index pattern and HTTP method are never derived from caller input."""
    resp = _search_hits_response([])
    client = ElkClient("http://elk:9200", index_pattern="filebeat-*")
    ...
    session.post = _post
    with patch.object(client, "_get_session", return_value=session):
        await client.search("web-01", ["/_cat/indices", "../../admin"])

    assert captured_urls[0] == "http://elk:9200/filebeat-*/_search"


@pytest.mark.asyncio
async def test_search_caps_max_results() -> None:
    resp = _search_hits_response([])
    client = ElkClient("http://elk:9200")
    ...
    def _post(url: str, json: dict[str, object], **kwargs: object) -> MagicMock:
        captured_bodies.append(json)
        return resp

    session.post = _post
    with patch.object(client, "_get_session", return_value=session):
        await client.search("web-01", ["error"], max_results=9999)

    assert captured_bodies[0]["size"] <= 50  # type: ignore[operator]


@pytest.mark.asyncio
async def test_search_filters_by_host() -> None:
    resp = _search_hits_response([])
    client = ElkClient("http://elk:9200")
    captured_bodies: list[dict[str, object]] = []
    ...
    session.post = _post
    with patch.object(client, "_get_session", return_value=session):
        await client.search("web-01", ["error"])

    body = captured_bodies[0]
    filter_clauses = body["query"]["bool"]["filter"]  # type: ignore[index]
    host_clause = next(c for c in filter_clauses if "bool" in c)
    terms = [s["term"]["host.name"] for s in host_clause["bool"]["should"] if "host.name" in s["term"]]
    assert terms == ["web-01"]


@pytest.mark.asyncio
async def test_search_no_results_empty_list() -> None:
    resp = _search_hits_response([])
    client = ElkClient("http://elk:9200")
    with patch.object(client, "_get_session", return_value=_mock_elk_session(resp)):
        result = await client.search("web-01", ["nothing"])
    assert result == []


@pytest.mark.asyncio
async def test_search_http_error_returns_empty() -> None:
    resp = _search_hits_response([], status=500)
    client = ElkClient("http://elk:9200")
    with patch.object(client, "_get_session", return_value=_mock_elk_session(resp)):
        result = await client.search("web-01", ["error"])
    assert result == []


@pytest.mark.asyncio
async def test_search_unreachable_returns_empty() -> None:
    import aiohttp
